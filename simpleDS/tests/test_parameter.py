...
def test_error_from_mixed_list():
    ...
    pytest.raises(ValueError, unp.UnitParameter, name='unp1',
...
def test_incompatible_tolerance_units():
    ...
    pytest.raises(units.UnitConversionError, unp.UnitParameter, name='unp1',
...
def test_error_raised_if_not_quantity_and_no_flag_set():
    ...
    pytest.raises(ValueError, unp.UnitParameter, name='unp1', value=3,
                  expected_units=units.m)
...
def test_unitparameters_equal_to_uvparameter_with_same_value():
    ...
    warn_message = ['A UnitParameter with quantity value is being cast to '
                    ...
                    'UVParameter units.']
    assert uvtest.checkWarnings(operator.eq, func_args=[unp1, uvp1],
                                category=[UserWarning] * len(warn_message),
                                nwarnings=len(warn_message),
                                message=warn_message)
    assert uvtest.checkWarnings(operator.eq, func_args=[uvp1, unp1],
                                category=[UserWarning] * len(warn_message),
                                nwarnings=len(warn_message),
                                message=warn_message)
...
def test_unitparameters_not_equal_to_uvp_with_different_type():
    ...
    warn_message = ['A UnitParameter with quantity value is being cast to '
                    ...
                    'to alter the unit of the value to match expected '
                    'UVParameter units.']
    assert uvtest.checkWarnings(operator.ne, func_args=[unp1, uvp1],
                                category=[UserWarning] * len(warn_message),
                                nwarnings=len(warn_message),
                                message=warn_message)
    assert uvtest.checkWarnings(operator.ne, func_args=[uvp1, unp1],
                                category=[UserWarning] * len(warn_message),
                                nwarnings=len(warn_message),
                                message=warn_message)
