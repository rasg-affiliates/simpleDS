...
def test_read_no_file():
    ...
    pytest.raises(TypeError, uvtest.checkWarnings,
...
def test_load_uv_no_antpos():
    ...
    pytest.raises(ValueError, uvtest.checkWarnings,
...
def test_no_antpos_file():
    ...
    pytest.raises(IOError, uvtest.checkWarnings,
                  utils.read_paper_miriad,
...
def test_antpos_from_file():
    ...
    assert np.allclose(test_uvws, test_uv.uvw_array)
...
def test_setting_frf_nebw_as_inttime():
    ...
    assert np.allclose(frf_nebw_array, test_uv.integration_time)
...
def test_bootstrap_array_invalid_axis():
    ...
    test_axis = 2
    nboot = 5
    pytest.raises(ValueError, utils.bootstrap_array, test_array,
...
def test_noise_equiv_bandwidth():
    ...
    assert np.isclose(2, 1. / utils.noise_equivalent_bandwidth(win),
                      rtol=1e-2)
...
def test_noise_equiv_bandwidth_boxcar():
    ...
    assert 1 == 1. / utils.noise_equivalent_bandwidth(win)
...
def test_cross_multiply_array_different_shapes():
    ...
    axis = 2
    pytest.raises(ValueError, utils.cross_multiply_array,
...
def test_noise_amplitude():
    ...
    assert np.isclose(test_noise.std(), 3,
...
def test_align_lst_error():
    ...
    pytest.raises(ValueError, utils.lst_align, test_uv, test_uv_2,
                  ra_range=[0, 24])
...
def test_delta_x_unitless():
    ...
    fake_data = np.zeros((1, 13, 21)) * units.m
    fake_data[0, 7, 11] += 1 * units.m
    pytest.raises(ValueError, utils.normalized_fourier_transform, fake_data,
                  delta_x=2., axis=2)
...
def test_combine_nsamples_different_shapes():
    ...
    pytest.raises(ValueError, utils.combine_nsamples,
...
def test_remove_autos_bad_axis_shape():
    ...
    axes = (1, 2, 3)
    pytest.raises(ValueError, utils.remove_auto_correlations, test_array,
                  axes=axes)
...
def test_weighted_average_unit_mismatch():
    ...
    pytest.raises(units.UnitConversionError, utils.weighted_average,
...
def test_weighted_average_weights_bad_shape_one_dimensional():
    ...
    weights = np.ones(4)
    pytest.raises(ValueError, utils.weighted_average, test_array,
...
def test_weighted_average_uniform_weights_value():
    ...
    assert np.array_equal(np.array([1, 1, 1, 1, 1]) / np.sqrt(test_array.shape[0]),
                          error_out.value)
...
def test_fold_along_delay_mismatched_sizes():
    ...
    axis = 2
    pytest.raises(ValueError, utils.fold_along_delay,
